"""
Filename: auto_corr_fft.py
------------------------------------------------------
This file shows how a pixel varies with time. It uses the fourier transform
...
def main():
    ...
    pix_cap_vector_fft = fft.fft(pix_cap_vector) * np.conj(fft.fft(pix_cap_vector))   # TODO Check to see if I need to transpose this. I think I do. Actually no I don't think I do.
    pix_bkgd_vector_fft = fft.fft(pix_bkgd_vector) * np.conj(fft.fft(pix_bkgd_vector))
    plt.plot(pix_cap_vector_fft[1:-1])
    plt.plot(pix_bkgd_vector_fft[1:-1])
    plt.show()

    # Create diagonal matrix for each pixel
    diag_cap = vector_to_diag(pix_cap_vector)
    diag_bkgd = vector_to_diag(pix_bkgd_vector)
    norm_try_pix = diag_cap[FRAME] #/ (np.mean(pix_cap_vector)**2)
    plt.plot(norm_try_pix)
    # plt.show()
    norm_try_bkgd = diag_bkgd[FRAME] #/ (np.mean(pix_bkgd_vector)**2)
